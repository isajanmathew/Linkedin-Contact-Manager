const DEFAULT_SUPABASE_URL = "https://wmmxdhjzpkuowqswscwv.supabase.co";
const DEFAULT_SUPABASE_PUBLISHABLE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6IndtbXhkaGp6cGt1b3dxc3dzY3d2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODU5NjMwNTAsImV4cCI6MjEwMTUzOTA1MH0.oUbXh1JIo1vjHGP6SRM_HKSxVUgL0reMwYcqQtVwHfY";
const log = console.log.bind(console, "[sync]");
const warn = console.warn.bind(console, "[sync]");
const logError = console.error.bind(console, "[sync]");
const KEYS = {
  contacts: "contacts",
  outbox: "outbox",
  meta: "syncMeta",
...
async function get$1(keys) {
  return chrome.storage.local.get(keys);
}
async function set(items) {
  return chrome.storage.local.set(items);
}
async function getDefaultQuarterlySetting() {
  const data = await get$1([KEYS.defaultQuarterlyReminder]);
  return data[KEYS.defaultQuarterlyReminder] !== void 0 ? Boolean(data[KEYS.defaultQuarterlyReminder]) : true;
}
async function setDefaultQuarterlySetting(enabled) {
  await set({ [KEYS.defaultQuarterlyReminder]: Boolean(enabled) });
}
async function getDeviceKey() {
  const data = await get$1([KEYS.deviceKey]);
  if (data[KEYS.deviceKey]) return data[KEYS.deviceKey];
  const defaultKey = "default_device";
  await set({ [KEYS.deviceKey]: defaultKey });
  return defaultKey;
}
async function setDeviceKey$1(key) {
  await set({ [KEYS.deviceKey]: (key || "").trim() });
}
async function getConnection() {
  const data = await get$1([KEYS.supabaseUrl, KEYS.supabaseAnonKey]);
  return {
    url: data[KEYS.supabaseUrl] || "",
    anonKey: data[KEYS.supabaseAnonKey] || ""
  };
}
async function setConnection$1({ url, anonKey }) {
  await set({
...
async function getContactMap() {
  const data = await get$1([KEYS.contacts]);
  return data[KEYS.contacts] || {};
}
async function setContactMap(map) {
  await set({ [KEYS.contacts]: map });
}
async function listContacts() {
  const map = await getContactMap();
  return Object.values(map).filter((c) => !c.deletedAt).sort((a, b) => (b.modifiedAt || "").localeCompare(a.modifiedAt || ""));
}
...
async function findContactByUrl(profileUrl) {
...
function sanitizeTags(tags) {
  const source = Array.isArray(tags) ? tags : String(tags || "").split(",").map((t) => t.trim());
  const seen2 = /* @__PURE__ */ new Set();
  const out = [];
  source.forEach((tag) => {
    const value = typeof tag === "string" ? tag.trim().slice(0, 100) : "";
    const dedupeKey = value.toLowerCase();
    if (value && !seen2.has(dedupeKey)) {
      seen2.add(dedupeKey);
      out.push(value);
    }
  });
...
async function saveContactLocal(input) {
  const map = await getContactMap();
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const existing = input.id && map[input.id] || Object.values(map).find(
...
async function markContactDeletedLocal(id) {
  const map = await getContactMap();
  const existing = map[id];
  if (!existing) return null;
  existing.deletedAt = (/* @__PURE__ */ new Date()).toISOString();
...
async function markSynced(id, remoteModifiedAt, remoteId) {
  const map = await getContactMap();
...
async function getOutbox() {
  const data = await get$1([KEYS.outbox]);
  return data[KEYS.outbox] || [];
}
async function setOutbox(ops) {
  await set({ [KEYS.outbox]: ops });
  await patchMeta({ pendingCount: ops.length });
}
async function enqueue(type, contactId) {
...
async function getMeta() {
  const data = await get$1([KEYS.meta]);
  return {
    lastPulledAt: null,
    lastPushedAt: null,
    lastError: null,
...
async function recordTagUsage(tags) {
  ...
  try {
    const data = await get$1([KEYS.tagUsage]);
...
async function getTagOptions() {
  const [contacts, data] = await Promise.all([listContacts(), get$1([KEYS.tagUsage])]);
  const usage = data[KEYS.tagUsage] || {};
...
const emptyToNull = (value) => {
  ...
  return trimmed === "" || trimmed === void 0 ? null : trimmed;
};
function toRow(contact, ownerKey) {
  return {
    id: contact.id,
    owner_key: ownerKey,
...
const localStore = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  KEYS,
  applyRemoteRows,
  enqueue,
  ...
  recordTagUsage,
  saveContactLocal,
  setConnection: setConnection$1,
  setContactMap,
  setDefaultQuarterlySetting,
  setDeviceKey: setDeviceKey$1,
  setOutbox,
  toRow
      ...
      while (true) {
        ...
        try {
          ...
          if (_this.retryEnabled && attemptCount < DEFAULT_MAX_RETRIES) {
            ...
            attemptCount++;
            await sleep$1(delay, _this.signal);
            continue;
          }
          throw fetchError;
        }
...
const PostgrestReservedCharsRegexp$1 = /* @__PURE__ */ new RegExp("[,()]");
...
var Socket = class {
  ...
  constructor(endPoint, opts = {}) {
    ...
    try {
      envSessionStorage = global$1 && global$1.sessionStorage;
    } catch {
    }
  ...
  async _performAuth(token, authGeneration) {
    ...
    if (this.accessTokenValue != tokenToSend) {
      ...
      this.channels.forEach((channel2) => {
        ...
        channel2.updateJoinPayload(payload);
...
async function getSupabase(ownerKey) {
...
async function ownerFingerprint(ownerKey) {
  ...
  try {
    ...
    contacts.forEach((contact) => {
      if (contact.followUpDate) {
        const followUp = /* @__PURE__ */ new Date(contact.followUpDate + "T23:59:59.999Z");
        ...
        if (baseDateStr) {
          ...
          if (!isNaN(base.getTime())) {
            const nextQuarter = new Date(base.getTime() + 90 * 24 * 60 * 60 * 1e3);
...
function isTransient(error) {
  ...
  return !navigator.onLine || message.includes("failed to fetch") || message.includes("network") || message.includes("timeout") || message.includes("econn") || message.includes("load failed") || message.includes("too many requests");
}
...
async function flushOutbox() {
  ...
  try {
    ...
    for (const op of ops) {
      ...
      try {
        if (op.type === "delete") {
          ...
          await markSynced(op.contactId, (/* @__PURE__ */ new Date()).toISOString());
        } else {
          const row = toRow(contact, ownerKey);
          const { id: _localId, ...updatable } = row;
          const { data: updated, error: updateError } = await supabase.from("contacts").update(updatable).eq("owner_key", ownerKey).eq("profile_url", row.profile_url).lte("modified_at", row.modified_at).select("id, modified_at");
          if (updateError) throw updateError;
          if (updated && updated.length > 0) {
            await markSynced(op.contactId, updated[0].modified_at, updated[0].id);
          } else {
            const { data: existing, error: existingError } = await supabase.from("contacts").select("id, modified_at").eq("owner_key", ownerKey).eq("profile_url", row.profile_url).maybeSingle();
            if (existingError) throw existingError;
            if (existing) {
              await markSynced(op.contactId, existing.modified_at, existing.id);
            } else {
              const { data: inserted, error: insertError } = await supabase.from("contacts").insert(row).select("id, modified_at").single();
    ...
    if (pushed && !remaining.length) {
      await patchMeta({ lastPushedAt: (/* @__PURE__ */ new Date()).toISOString(), lastError: null, connected: true });
    }
...
async function pullChanges({ full = false } = {}) {
  ...
  try {
    ...
    await patchMeta({
      lastPulledAt: newest || (/* @__PURE__ */ new Date()).toISOString(),
      lastError: null,
      connected: true
    });
...
async function subscribeRealtime() {
  ...
  channel = supabase.channel(topicName).on(
    "postgres_changes",
    {
      event: "INSERT",
      schema: "public",
...
async function setDeviceKey(key) {
  const { setDeviceKey: persist } = await __vitePreload(async () => {
    const { setDeviceKey: persist2 } = await Promise.resolve().then(() => localStore);
    return { setDeviceKey: persist2 };
  }, true ? void 0 : void 0);
  await persist(key);
  resetSupabase();
  await teardownRealtime();
...
async function setConnection({ url, anonKey }) {
  const { setConnection: persist, getDeviceKey: readKey } = await __vitePreload(async () => {
    const { setConnection: persist2, getDeviceKey: readKey2 } = await Promise.resolve().then(() => localStore);
    return { setConnection: persist2, getDeviceKey: readKey2 };
  }, true ? void 0 : void 0);
...
async function handleMessage(request) {
  switch (request.action) {
    ...
    case "setDefaultSettings": {
      if (request.defaultQuarterlyReminder !== void 0) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  const enriched = request?.action === "openSidePanelFromButton" ? { ...request, tabId: request.tabId || sender.tab?.id } : request;
  handleMessage(enriched).then(sendResponse).catch((error) => {
